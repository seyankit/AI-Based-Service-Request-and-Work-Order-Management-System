...
function mapServiceRequestFromApi(record) {
  return {
    ...
    requesterType:
      state.currentAccount?.personnelType || "",

    department:
      record.requesterDepartmentName ||
      state.currentAccount?.department ||
      "",

    requestedCategoryId:
      record.requestedCategoryId ?? null,
...
function mapAdministratorReviewFromApi(item) {
  ...
  const recommendedCategory =
    ai.recommendedCategoryName ||
    ai.categoryName ||
    "";

  const recommendedPriority =
    ai.recommendedPriority ||
    "";

  const duplicateRequestNumber =
    ai.duplicateCandidateRequestNumber ||
    "";

  const similarityValue =
    Number(
      ai.duplicateSimilarityScore ??
      ai.similarityScore
    );
  ...
  return {
    ...
    aiRecommendationMethod:
      ai.recommendationMethod || ""
  };
}
...
async function loadAdministratorServiceRequestReviews() {
  try {
    ...
    if (
      ...
      throw new Error(
        data.message ||
        "Unable to load the Administrator request queue."
      );
    }
    ...
    state.adminReviewRequests =
      Array.isArray(
        ...
        ? data.reviewItems.map(
            mapAdministratorReviewFromApi
          )
        : [];
    ...
    console.error(
      "Unable to load Administrator request queue:",
      error
    );
...
async function loadRequesterServiceRequests() {
  ...
  try {
    const response = await fetch(
      `${API_ENDPOINTS.serviceRequests}?limit=100`,
      {
        method: "GET",
        headers: {
          Accept: "application/json"
...
async function loadRequesterServiceRequestDrafts() {
  ...
  try {
    ...
    if (!response.ok || !data.success) {
      throw new Error(
        data.message ||
        "Unable to load service request drafts."
      );
    }
...
function findRequest(
  ...
  return (
    ...
    state.adminReviewRequests.find(
      (request) =>
        request.id === requestId
    )
  );
...
function openConfirm({
  ...
  byId("confirmActionButton").className = `button ${type === "danger" ? "button-danger" : "button-primary"}`;
  ...
  openModal("confirmModal", requireRemarks ? "#confirmRemarks" : "#confirmActionButton");
}
...
function openWorkspaceView(
  ...
  byId(
    ...
    `Service Portal / ${roleConfiguration[
      state.activeRole
    ].label
    } / ${title}`;
...
function renderWorkspaceAvatar(
  ...
  image.src =
    `${API_ENDPOINTS.profilePhoto}?v=${encodeURIComponent(profileImageFileName)}`;
...
function activateRoleWorkspace(
  ...
  if (
    ...
    showToast(
      "This account role is not available.",
      "error"
    );
  ...
  const workspaceEmail =
    byId("workspaceUserEmail");

  if (workspaceEmail) {
    workspaceEmail.textContent =
      email;

    workspaceEmail.title =
      email;

    workspaceEmail.setAttribute(
      "aria-label",
...
function renderPublicMetrics() {
  ...
  byId("publicTotalMetric").textContent =
    String(total).padStart(2, "0");

  byId("publicPendingMetric").textContent =
    String(pending).padStart(2, "0");

  byId("publicProgressMetric").textContent =
    String(progress).padStart(2, "0");

  byId("publicCompletedMetric").textContent =
...
async function deleteServiceRequestDraft(draftId) {
  ...
  if (
    !Number.isInteger(normalizedDraftId) ||
    normalizedDraftId <= 0
  ) {
    showToast(
...
function confirmDeleteServiceRequestDraft(
  draftId
) {
  const normalizedDraftId =
    Number(draftId);
  ...
  const draft =
    state.serviceRequestDrafts.find(
      (item) =>
        Number(item.draftId) ===
        normalizedDraftId
    );
  ...
  openConfirm({
    ...
    message:
      `${draftName} will be permanently removed from your saved drafts.`,
...
function resumeServiceRequestDraft(draftId) {
  ...
  window.setTimeout(
    () => {
      ...
      byId("requestPriority").value =
        draft.preferredPriority || "";

      byId("requestTitle").value =
        draft.title || "";

      byId("requestDescription").value =
        draft.description || "";

      byId("requestLocation").value =
        draft.location || "";

      byId("requestDate").value =
        draft.dateReported || "";

      byId("requestCharacterCount").textContent =
        String(
          (draft.description || "").length
        );

      state.currentDraftId =
        Number(draft.draftId);

      const draftButton =
        byId("serviceRequestDraftButton");
...
function renderRequesterDrafts() {
  const list =
    byId("requesterDraftList");

  const count =
    byId("requesterDraftCount");

  const emptyState =
    byId("requesterDraftEmptyState");
  ...
  const drafts =
    Array.isArray(state.serviceRequestDrafts)
      ? state.serviceRequestDrafts
      : [];
  ...
  drafts.forEach((draft) => {
    ...
    details.textContent =
      `Draft #${draft.draftId} | Updated ${updated}`;
    ...
    resumeButton.dataset.resumeDraft =
...
function renderPersonnelTable() {
  ...
  byId(
    ...
    `${state.personnel.length} ${state.personnel.length === 1
      ? "Record"
      : "Records"
    }`;
}
...
function renderApprovalHistory() {
  ...
  byId(
    ...
    `${state.approvalHistory.length} ${state.approvalHistory.length ===
      1
      ? "Record"
      : "Records"
    }`;
...
function renderTechnicianHistory() {
  ...
  byId(
    ...
    `${state.technicianHistory.length} ${state.technicianHistory.length ===
      1
      ? "Record"
      : "Records"
    }`;
...
function showDetails(
  ...
  const label =
    byId("detailsModalLabel");

  const title =
    byId("detailsModalTitle");

  const content =
    byId("detailsModalContent");
  ...
  if (
    ...
    console.error(
      "Details modal elements are missing."
    );
...
async function loadRequesterAttachmentsForDetails(
  requestId
) {
...
async function showRequestDetails(
  ...
  if (
    ...
    request.databaseId != null
  ) {
    try {
      const attachments =
        await loadRequesterAttachmentsForDetails(
          request.databaseId
        );
      ...
      console.error(
        "Request attachment details error:",
        error
      );
...
function setFieldSuccess(input) {
  ...
  if (input.dataset.touched === "true" && input.type !== "file") group?.classList.add("is-valid");
...
function validateEmail(input) {
  ...
  if (!SCHOOL_EMAIL_PATTERN.test(value)) {
    return setFieldError(input, "Enter a valid Holy Trinity College school email ending in @online.htcgsc.edu.ph.");
  }
  return setFieldSuccess(input);
}
...
function updatePasswordStrength() {
  ...
  if (!input.value) {
    text.textContent = "Use uppercase, lowercase, number, and special character.";
    return;
  }
...
function validatePassword(input) {
  ...
  if (value !== value.trim()) return setFieldError(input, "Password cannot begin or end with spaces.");
  if (!PASSWORD_PATTERN.test(value)) {
    return setFieldError(input, "Use at least 8 characters with uppercase, lowercase, number, and special character.");
  }
  return setFieldSuccess(input);
}
...
function validatePasswordMatch(input) {
  ...
  if (input.value !== byId("registerPassword")?.value) return setFieldError(input, "Passwords do not match.");
  return setFieldSuccess(input);
}
...
function validateSelect(input) {
  if (!input.value && input.required) return setFieldError(input, "Select a valid option.");
  return setFieldSuccess(input);
}

function validateDateNotFuture(input, required = input.required) {
  if (!input.value) {
    return required ? setFieldError(input, "Select a valid date.") : setFieldSuccess(input);
  }
  if (input.value > todayInputValue()) return setFieldError(input, "Date cannot be set in the future.");
  return setFieldSuccess(input);
}
...
function validateTextInput(input) {
  ...
  if (input.required && !value) return setFieldError(input, "This field is required and cannot contain only spaces.");
  if (!value && !input.required) return setFieldSuccess(input);

  const minLength = Number(input.getAttribute("minlength") || 0);
  const maxLength = Number(input.getAttribute("maxlength") || 0);
  if (minLength && !hasMeaningfulText(value, minLength)) return setFieldError(input, `Enter at least ${minLength} meaningful characters.`);
  if (maxLength && value.length > maxLength) return setFieldError(input, `Use no more than ${maxLength} characters.`);
  if (/^(.)\1{5,}$/i.test(value.replace(/\s/g, ""))) return setFieldError(input, "Enter meaningful information instead of repeated characters.");
  input.value = value;
  return setFieldSuccess(input);
}
...
function validateIdPattern(input, prefix) {
  ...
  if (!pattern.test(value)) return setFieldError(input, `Use the format ${prefix}-YYYY-NNNN.`);
  return setFieldSuccess(input);
}

function validateControl(input) {
  if (!input || input.disabled || input.type === "hidden" || input.type === "submit" || input.type === "button" || input.type === "reset" || input.type === "checkbox") return true;

  if (input.tagName === "SELECT") return validateSelect(input);
  if (input.type === "email") return validateEmail(input);
  if (["registerFirstName", "registerMiddleName", "registerLastName", "requesterName", "personnelName"].includes(input.id)) return validateName(input);
  if (input.id === "registerPassword") return validatePassword(input);
  if (input.id === "registerConfirmPassword") return validatePasswordMatch(input);
  if (input.type === "tel") return validateContactNumber(input);
  if (input.id === "loginPassword") {
    if (!input.value.trim()) return setFieldError(input, "Password is required.");
    return setFieldSuccess(input);
  }
  if (["requestDate", "completionDate", "progressCompletionDate"].includes(input.id)) return validateDateNotFuture(input, input.required);
  if (["workOrderId", "progressWorkOrderId"].includes(input.id)) return validateIdPattern(input, "WO");
  if (["workOrderRequestId", "progressRequestId"].includes(input.id)) return validateIdPattern(input, "SR");
...
function setAuthMode(mode) {
  ...
  clearAuthMessage();
  window.setTimeout(() => byId(registerMode ? "registerFirstName" : "loginEmail")?.focus(), 20);
}

const EMAIL_VERIFICATION_STORAGE_KEY =
  "htcEmailVerificationComplete";
...
function showEmailVerificationSuccess() {
  ...
  const icon =
    byId("verificationStatusIcon");

  const title =
    byId("verificationPanelTitle");

  const message =
    byId("verificationPanelMessage");

  const noticeTitle =
    byId("verificationEmailDisplay");

  const description =
    byId("verificationStatusDescription");

  const resendButton =
    byId("resendVerificationButton");

  const signInButton =
    byId("verificationSignInButton");

  const backButton =
    byId("verificationBackToLogin");
...
function handleEmailVerificationReturn() {
  ...
  const result =
    currentUrl.searchParams.get("verified");
  ...
  if (result === "success") {
    ...
    try {
      ...
      console.warn(
        "Unable to notify another tab about email verification."
      );
    }
...
function initializeEmailVerificationSync() {
  window.addEventListener(
    ...
    (event) => {
      if (
        event.key !== EMAIL_VERIFICATION_STORAGE_KEY ||
        !event.newValue
      ) {
        return;
      }
...
function validateAttachment(input, report = true) {
  ...
  if (!allowedTypes.includes(file.type) || !allowedExtensions.test(file.name)) {
    setFieldError(input, "Upload a PNG, JPG, WEBP, or PDF file no larger than 5 MB.");
...
function validateCompletionDate(statusInput, dateInput, actionInput = null) {
  ...
  if (statusInput.value === "Completed" && !dateInput.value) {
    setFieldError(dateInput, "Enter a completion date when the work status is Completed.");
    return false;
  }
  ...
  if (["In Progress", "Completed"].includes(statusInput.value) && actionInput && !hasMeaningfulText(actionInput.value, 5)) {
    setFieldError(actionInput, "Describe the action taken using at least 5 meaningful characters.");
    return false;
  }
...
function restoreRememberedUser() {
  try {
    const saved = JSON.parse(localStorage.getItem(REMEMBER_STORAGE_KEY) || "null");
    if (!saved) return;
...
function roleValueToId(value) {
  ...
  return ROLE_ID_BY_KEY[String(value || "").trim().toLowerCase()] || 0;
}
...
async function loadDepartments() {
  ...
  try {
    ...
    if (departments.some((department) => String(department.id) === previousValue)) {
...
function accountFromAuthenticationData(data) {
  ...
  return {
    ...
    profileImageFileName:
      String(data.profileImageFileName || "").trim(),
    roleId,
    role
  };
}
...
async function restoreServerSession() {
  try {
    ...
    if (!response.ok || !data.success || !data.authenticated) {
      console.warn("Session restoration was not completed:", data.message || response.status);
      return;
    }
    ...
    if (!account) {
      ...
      console.error("The active session contains an unsupported role assignment.");
      return;
    }
    ...
    if (account.role === "administrator") {
      try {
        ...
        console.warn(
          "Administrator request queue could not be loaded:",
          error
        );
      }
    }
...
async function handleRegistrationSubmit(event) {
  ...
  try {
    const response = await fetch(API_ENDPOINTS.register, {
      ...
      credentials: "same-origin",
      body: JSON.stringify({ firstName, middleName, lastName, suffix, email, contactNumber, personnelType, password, departmentId })
    });
    const data = await readJsonResponse(response);
    if (!response.ok || !data.success) {
      const message = data.message || "Unable to create account.";
      if (response.status === 409) { setFieldError(byId("registerEmail"), message); byId("registerEmail").focus(); return; }
      showAuthMessage(message, "error"); return;
    }

    state.pendingVerificationEmail = email;
    byId("verificationEmailDisplay").textContent = data.maskedEmail ? `Verification sent to ${data.maskedEmail}` : "Check your school email";
    form.hidden = true;
    byId("loginForm").hidden = true;
    byId("verificationPanel").hidden = false;
    showAuthMessage(data.message || "Check your school email to verify your account.", data.mailSent === false ? "error" : "success");
    form.reset(); resetValidationState(form); updatePasswordStrength();
  } catch (error) {
    console.error("Registration error:", error);
    showAuthMessage("Unable to connect to the server. Please make sure Tomcat is running.", "error");
  } finally { setSubmitting(form, false); }
}

async function resendVerificationEmail() {
  const email = state.pendingVerificationEmail;
  if (!email) { showAuthMessage("Enter your school email again to request verification.", "error"); return; }
  const button = byId("resendVerificationButton");
  if (button) button.disabled = true;
  try {
    const response = await fetch(API_ENDPOINTS.resendVerification, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, credentials: "same-origin", body: JSON.stringify({ email }) });
    const data = await readJsonResponse(response);
    showAuthMessage(data.message || "Verification request processed.", response.ok && data.success ? "success" : "error");
  } catch (error) { showAuthMessage("Unable to resend verification email.", "error"); }
  finally { window.setTimeout(() => { if (button) button.disabled = false; }, 60000); }
}
...
async function handleServiceRequestDraftSave() {
  ...
  const payload =
    buildServiceRequestDraftPayload();
  ...
  try {
    ...
    if (existingIndex >= 0) {
      ...
      state.serviceRequestDrafts.unshift(
        data.draft
      );
    }
...
async function handleServiceRequestSubmit(event) {
  ...
  const payload = {
    ...
    description:
      byId("requestDescription")
        .value
        .trim(),

    location:
      byId("requestLocation")
        .value
        .trim(),
  ...
  try {
    ...
    if (selectedAttachment) {
      ...
      if (
        ...
        try {
          ...
          console.error(
            "Service request attachment upload error:",
            error
          );
    ...
    if (draftIdToDeleteAfterSubmission) {
      try {
        ...
        if (
          ...
          throw new Error(
            draftDeleteData.message ||
            "The saved draft could not be removed."
          );
        }

        if (
          Number(state.currentDraftId) ===
          draftIdToDeleteAfterSubmission
        ) {
          resetServiceRequestDraftEditingState();
        }
      } catch (error) {
        console.error(
          "Submitted request draft cleanup error:",
          error
        );
    ...
    if (
      ...
      if (draftCleanupError) {
        warnings.push(
          `the saved draft was not removed (${draftCleanupError})`
        );
      }
      ...
      showToast(
        `${createdRequestNumber} submitted successfully.`,
        "success"
      );
    }
...
function populatePersonnelDepartmentOptions(
  selectedDepartmentId = null
) {
  const select =
    byId("personnelDepartment");
...
function populatePersonnelAccessDetails(
  personnelId
) {
  const numericPersonnelId =
    Number(personnelId);
  ...
  const type =
    byId("personnelTypeDisplay");

  const status =
    byId("personnelStatusDisplay");
  ...
  if (role) {
    role.value =
      person.roleId == null
        ? ""
        : String(person.roleId);
  }
}

function populatePersonnelAccessManagement(
  selectedPersonnelId = null
) {
  const select =
    byId("personnelTargetId");
  ...
  if (
    ...
    populatePersonnelAccessDetails(
      selectedPersonnelId
    );
  } else {
    clearPersonnelAccessFields();
...
async function handlePersonnelSubmit(
  ...
  if (
    ...
    showToast(
      "Select a personnel account to manage.",
      "error"
    );
...
async function openForwardRequestModal(
  request
) {
  const form =
    byId(
  ...
  const categorySelect =
    byId(
      "adminForwardCategory"
    );

  const prioritySelect =
    byId(
      "adminForwardPriority"
    );

  const departmentSelect =
    byId(
      "adminForwardDepartment"
    );
  ...
  if (
    ...
    showToast(
      "The Forward Request form is unavailable.",
      "error"
    );
  ...
  byId(
    "adminForwardRequestLabel"
  ).textContent =
    `${request.id} - ${request.title}`;

  byId(
    "adminForwardRequestedCategory"
  ).textContent =
  ...
  try {
    const {
      ...
      await loadForwardReferenceData();
    ...
    console.error(
      "Forward reference data failed:",
      loadError
    );

    error.textContent =
      loadError.message ||
      "Unable to prepare the Forward Request form.";
...
async function forwardRequest(
  ...
  if (
    ...
    error.textContent =
      "The database request identifier is unavailable.";
  ...
  if (
    ...
    error.textContent =
      "Select the final service category.";
  ...
  if (
    ...
    routedDepartmentId <= 0
  ) {
    error.textContent =
      "Select the routed department.";
  ...
  try {
    ...
    const response =
      await fetch(
        API_ENDPOINTS.serviceRequestReviews,
        {
          method: "PUT",
    ...
    if (
      ...
      throw new Error(
        data.message ||
        "Unable to forward the service request."
      );
    }
    ...
    console.error(
      "Service-request forwarding failed:",
      forwardError
    );
...
async function openDuplicateRequestModal(
  request
) {
  const form =
    byId(
  ...
  if (
    ...
    showToast(
      "The duplicate confirmation form is unavailable.",
      "error"
    );
  ...
  try {
    ...
    console.error(
      "Duplicate candidate loading failed:",
      loadError
    );
...
async function flagDuplicate(
  ...
  if (
    ...
    error.textContent =
      "The duplicate request identifier is unavailable.";
  ...
  if (
    confirmationReason.length > 1000
  ) {
    error.textContent =
  ...
  try {
    ...
    const response =
      await fetch(
        API_ENDPOINTS.serviceRequestDuplicates,
        {
          method:
            "POST",
    ...
    showToast(
      `${request.id} was confirmed as a duplicate.`,
      "success"
    );
...
========================= */
function setProfilePhotoControlsBusy(
  busy
) {
  [
...
async function handleProfilePhotoFileChange(
  event
) {
  const input =
    event.currentTarget;
...
function initializeEvents() {
  ...
  on(
    ...
    () => {
      ...
      if (state.activeRole) {
        showToast("Request tracking is available from a Requester account.", "info");
        return;
      }
      setAuthMode("login");
  ...
  on(byId("togglePasswordButton"), "click", () => togglePasswordVisibility("loginPassword", "togglePasswordButton"));
  on(byId("toggleRegisterPassword"), "click", () => togglePasswordVisibility("registerPassword", "toggleRegisterPassword"));
  on(byId("toggleConfirmPassword"), "click", () => togglePasswordVisibility("registerConfirmPassword", "toggleConfirmPassword"));
  ...
  on(byId("registerPassword"), "input", () => {
    ...
    if (confirmInput.dataset.touched === "true" || getFieldGroup(confirmInput)?.classList.contains("is-invalid")) {
      validatePasswordMatch(confirmInput);
    }
  });

  all(".record-form input, .record-form select, .record-form textarea").forEach((input) => {
    if (["checkbox", "submit", "button", "reset"].includes(input.type)) return;
    ...
    on(input, input.tagName === "SELECT" || input.type === "file" ? "change" : "input", () => {
      ...
      if (input.dataset.touched === "true" || getFieldGroup(input)?.classList.contains("is-invalid")) {
        validateControl(input);
      }
    });
  });
  ...
  on(
    ...
    () => {
      ...
      if (state.confirmRequiresRemarks && !hasMeaningfulText(remarks, 5)) {
        byId("confirmRemarksError").textContent = "Enter at least 5 meaningful characters explaining the rejection.";
  ...
  on(
    byId(
      "serviceRequestDraftButton"
    ),
    "click",
    handleServiceRequestDraftSave
  );
  ...
  on(
    ...
    (event) => {
      ...
      if (deleteButton) {
        confirmDeleteServiceRequestDraft(
          deleteButton.dataset.deleteDraft
        );
...
on(
  ...
  on(
    byId("workOrderForm"),
    "reset",
    (event) => resetValidationState(event.currentTarget)
  );
...
function animateMetricElement(element) {
  ...
  const step = (now) => {
    ...
    element.textContent = pad ? String(value).padStart(pad, "0") : String(value);
...
function initializeMetricAnimations() {
  [
    "publicTotalMetric", "publicPendingMetric", "publicProgressMetric", "publicCompletedMetric",
    "requesterTotalMetric", "requesterPendingMetric", "requesterProgressMetric", "requesterCompletedMetric",
    "adminQueueMetric", "adminApprovalMetric", "adminHighMetric", "adminWorkOrderMetric",
    "approverPendingMetric", "approverApprovedMetric", "approverRejectedMetric",
    "technicianAssignedMetric", "technicianProgressMetric", "technicianHoldMetric", "technicianCompletedMetric"
...
async function initializeApp() {
   ...
   */
  const verificationReturnHandled =
  ...
  if (byId("workspaceDate")) {
    byId("workspaceDate").textContent =
